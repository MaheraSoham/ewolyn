      ...
      <div className="container-max py-16 relative z-10">
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-12">
                ...
                <ul className="space-y-4">
                  {scheme.eligibility.map((item, idx) => (
                    <li key={idx} className={`flex items-start gap-4 p-3 rounded-xl ${idx % 2 === 0 ? "bg-slate-50" : ""}`}>
                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary text-xs font-bold mt-0.5">{idx + 1}</span>
            ...
            <section>
              <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-3">
                <svg className="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
                Key Benefits
              </h2>
              <div className="grid gap-4 md:grid-cols-2">
                {scheme.benefits.map((item, idx) => (
                  <div key={idx} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm group hover:shadow-lg hover:border-primary/20 hover:-translate-y-0.5 transition-all duration-300">
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 shrink-0 rounded-xl bg-primary/10 text-primary flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors">
                        {benefitIcons[idx % benefitIcons.length]}
                      </div>
                      <div>
                        <span className="text-xs text-primary font-bold uppercase tracking-wider">Benefit {idx + 1}</span>
